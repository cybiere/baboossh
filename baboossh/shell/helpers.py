...
def get_option_creds(shell):
    return [cmd2.CompletionItem("#"+str(creds.id), display_meta=creds.obj.toList()) for creds in shell.workspace.get_objects(creds=True, scope=True)]

def get_option_host(shell):
    return [cmd2.CompletionItem(str(host.name), display_meta="; ".join(str(e) for e in host.endpoints)) for host in shell.workspace.get_objects(hosts=True, scope=True)]
...
def get_option_user(shell):
    return [cmd2.CompletionItem(str(user), display_meta=str(user)) for user in shell.workspace.get_objects(users=True, scope=True)]

def get_option_endpoint(shell):
    return [cmd2.CompletionItem(str(endpoint), display_meta="" if endpoint.host == None else str(endpoint.host)) for endpoint in shell.workspace.get_objects(endpoints=True, scope=True)]

def get_option_endpoint_tag(shell):
    endpoints = [cmd2.CompletionItem(str(endpoint), display_meta="" if endpoint.host == None else str(endpoint.host)) for endpoint in shell.workspace.get_objects(endpoints=True, scope=True)]
    tags = [cmd2.CompletionItem("!"+str(tag.name), display_meta="; ".join(str(e) for e in tag.endpoints)) for tag in shell.workspace.get_objects(tags=True, scope=True)]
...
def get_option_connection(shell):
    return [cmd2.CompletionItem(str(connection), display_meta="" if connection.endpoint.host == None else str(connection.endpoint.host)) for connection in shell.workspace.get_objects(connections=True, scope=True)]
...
def get_open_tunnels(shell):
    return [cmd2.CompletionItem(str(tunnel.port), display_meta=str(tunnel.connection)) for tunnel in shell.workspace.get_objects(tunnels=True)]
...
def get_host_or_local(shell):
    ...
    return items+[cmd2.CompletionItem(str(host.name), display_meta="; ".join(str(e) for e in host.endpoints)) for host in shell.workspace.get_objects(hosts=True, scope=True)]

def get_endpoint_or_host(shell):
    return get_option_host(shell) + get_option_endpoint(shell)

def get_tag(shell):
    return [cmd2.CompletionItem(str(tag.name), display_meta="; ".join(str(e) for e in tag.endpoints)) for tag in shell.workspace.get_objects(tags=True, scope=True)]
